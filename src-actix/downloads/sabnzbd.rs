...
pub async fn pause_queue(url: &str, api_key: &str) -> color_eyre::eyre::Result<()> {
...
pub async fn resume_queue(url: &str, api_key: &str) -> color_eyre::eyre::Result<()> {
    let client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(10))
        .build()?;
...
pub async fn fetch_downloads(url: &str, api_key: &str) -> color_eyre::eyre::Result<ClientDownloads> {
    let client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(10))
        .build()?;
